...
class SketchField extends PureComponent {
  ...
  zoom = (factor) => {
    let canvas = this._fc;
    let objects = canvas.getObjects();
    let back = canvas.backgroundImage;
    back.scaleX = back.scaleX*factor;
    back.scaleY = back.scaleY*factor;
    back.left = (back.left-canvas.width/2)*factor+canvas.width/2;
    back.top = (back.top-canvas.height/2)*factor+canvas.height/2;
    for (let i in objects) {
      objects[i].scaleX = objects[i].scaleX * factor;
      objects[i].scaleY = objects[i].scaleY * factor;
      objects[i].left = (objects[i].left-canvas.width/2) * factor+canvas.width/2;
      objects[i].top = (objects[i].top-canvas.height/2) * factor+canvas.height/2;
      objects[i].setCoords();
    }
    canvas.renderAll();
    canvas.calcOffset();
  ...
  setBackgroundFromDataUrl = (dataUrl, options = {}) => {
    ...
    img.onload = () => {
      ...
      if (canvas.width>=canvas.height) imgObj.scaleToWidth(canvas.width)
      else imgObj.scaleToWidth(canvas.width)
      imgObj.set({
        "top": canvas.height/2-(canvas.width/imgObj.width)*imgObj.height/2
      })
  ...
  componentDidUpdate = (prevProps, prevState) => {
    ...
    if(this.props.lineWidth !== prevProps.lineWidth){
      this._lineWidth = this.props.lineWidth
      this._selectedTool.configureCanvas(this.props);
    }

    if(this.props.lineColor !== prevProps.lineColor){
      this._lineColor = this.props.lineColor
      this._selectedTool.configureCanvas(this.props);
    }
