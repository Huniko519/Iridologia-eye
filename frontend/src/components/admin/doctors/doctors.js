...
import { Table, Space, Input, Row, Col, Modal, Form, message } from 'antd';
...
const App = (props) => {
          ...
          patient_temp.push(
            {
              ...
              key: index + 1,
              name: patient.name,
              password: patient.originPassword,
              email: patient.email,
              membership1: patient.membership1,
              membership2: patient.membership2,
              membership3: patient.membership3,
              space: 0.05 + patient.membership1 * 0.25 + patient.membership2 + patient.membership3*0.6+ "GB"
            }
          )
        })
        console.log(patient_temp[0].password)
  ...
  const columns = [
    ...
    {
      title: landingPageData ? landingPageData.Register.password : "loading",
      dataIndex: 'password',
      key: 'password',
    ...
    {
      title: landingPageData ? landingPageData.AdminNavbar.recharge : "loading",
      dataIndex: 'membership3',
      key: 'membership3',
      sorter: (a, b) => a.membership2.localeCompare(b.membership3),
      render: text => <em>{text}</em>,
    },
    {
  ...
  const onHandleDialog = (key)=>{
    setDialogOpen(true);
    setPatientKey(key);
  }
  const offHandleDialog = ()=>{
    setDialogOpen(false);
  }
  ...
  return (
    <div>
      ...
      <>
        <Dialog
          open={dialogOpen}
          onClose={offHandleDialog}
          aria-labelledby="alert-dialog-title"
          aria-describedby="alert-dialog-description"
        >
          <DialogTitle id="alert-dialog-title">{landingPageData ? landingPageData.PatientTable.deleteConfirmUser : "loading"}</DialogTitle>
          <DialogActions>
            <Button onClick={onDelete} color="primary">
              {landingPageData ? landingPageData.PatientTable.delete : "loading"}
            </Button>
            <Button onClick={offHandleDialog} color="primary" autoFocus>
              {landingPageData ? landingPageData.PatientTable.cancel : "loading"}
            </Button>
          </DialogActions>
        </Dialog>
        <Modal
        ...
        <Row style={{ padding: '8px', paddingTop: '30px' }}>
